...
import WorkspaceFinal from "@/components/v15/WorkspaceFinal";
import PresenceController from "@/components/v15/PresenceController";
import "./v15.css";
import "./v15-extra.css";
...
export default function Home(){
  ...
  useEffect(()=>{if(!firebaseConfigured){setReady(true);return;}return onAuthStateChanged(auth,u=>{setUser(u);setReady(true)})},[]);
  async function submit(e:React.FormEvent){e.preventDefault();const name=normalizeUsername(username);setError("");if(!validUsername(name)){setError("Kullanıcı adı 3-24 karakter olmalı: a-z, 0-9, _ veya -.");return}if(password.length<6){setError("Şifre en az 6 karakter olmalı.");return}setBusy(true);try{if(register){const cred=await createUserWithEmailAndPassword(auth,usernameEmail(name),password);const now=Date.now();const shown=displayName.trim()||name;await set(ref(db,`users/${cred.user.uid}/public`),{username:name,usernameLower:name,displayName:shown,createdAt:now,status:"online",statusText:""});await set(ref(db,`usernameIndex/${name}`),cred.user.uid);await createStarterServer(cred.user.uid)}else await signInWithEmailAndPassword(auth,usernameEmail(name),password)}catch(err){const code=(err as {code?:string})?.code||"";setError(code.includes("email-already")?"Bu kullanıcı adı zaten alınmış.":code.includes("invalid-credential")||code.includes("wrong-password")?"Kullanıcı adı veya şifre hatalı.":code.includes("network")?"Bağlantı hatası.":"İşlem başarısız. Tekrar dene")}finally{setBusy(false)}}
  if(!firebaseConfigured)return <State text="Firebase yapılandırması bulunamadı."/>;
  if(!ready)return <State text="AkayRoom bağlanıyor…"/>;
  if(user)return <><PresenceController user={user}/><WorkspaceFinal user={user}/></>;
